/**
 * @copyright Copyright (c) Open-Xchange GmbH, Germany <info@open-xchange.com>
 * @license AGPL-3.0
 *
 * This code is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with OX App Suite. If not, see <https://www.gnu.org/licenses/agpl-3.0.txt>.
 *
 * Any use of the work other than as authorized under this license or copyright law is prohibited.
 */

import $ from '$/jquery'
import ox from '$/ox'

// application object. 'name' is mandatory!
const app = ox.ui.createApp({ name: 'com.example/my-simple-app' })

// launcher
app.setLauncher(options => {
  // application window (some applications don't have a window)
  const win = ox.ui.createWindow({
    name: 'com.example/my-simple-app',
    title: 'My simple app'
  })

  app.setWindow(win)

  // Add css class with your namespace
  win.addClass('com-example-my-simple-app')

  // add something on 'main' node
  win.nodes.main
    .css({ padding: '13px', textAlign: 'center' })
    .append(
      $('<h1>').text('My simple app'),
      $('<p>').text('Hello World!')
    )

  // show the window
  win.show()

  // When loading is complete just add the complete class to the html node
  $('html').addClass('complete')
})

export default {
  getApp: app.getInstance
}
